...
| **M1** | **Prove data engineering ability, harder** | The author's first concern, and the site's biggest gap. **Partly landed.** Done: job bullets naming systems, volumes and failure modes, each with its consequence on a `.point__impact` line; Home's *Currently* block; Impact in Numbers quoting those bullets rather than paraphrasing them; and now the **diagram mechanism**, which draws a declared architecture as inline SVG with no JavaScript, prints, and inherits the page's ink ([`diagrams.md`](diagrams.md)). **Two diagrams now exist**, declared by the author and then narrowed to the nouns `experience.json` and `research.json` actually carry: the JACQUEMUS order path on Career, the Ausgrid pipeline on Research. Still open, and **author-led**: pipeline case studies (problem, architecture, trade-offs, what broke, what changed), and possibly a *How I work* page from the doctrine now in Career's Summary. **Do not auto-generate this content**, and when an author's sketch names systems the records do not, that is a question for the author, not a gap for a renderer to fill ([`diagrams.md`](diagrams.md)). |
